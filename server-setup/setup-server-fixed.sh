...
# Script de configuration du serveur de mise à jour Karlog (CORRIGÉ)
# Port 3001 (Traefik utilise 8080) et sans SSL

set -e

echo "🚀 Configuration du serveur de mise à jour Karlog (Port 3001 - HTTP simple)..."
...
# Configuration Nginx HTTP simple (SANS SSL)
echo "⚙️  Configuration de Nginx (HTTP sur port $PORT)..."
cat > $NGINX_CONF << EOF
# Configuration Nginx pour Karlog Updates - Port $PORT HTTP
server {
    listen $PORT;
    server_name _;
...
# Création d'une page de statut HTML
...
<body>
    <div class="container">
        ...
        <div class="info">
            ...
            <button class="btn" onclick="testMeta()">Tester métadonnées</button>
            <div id="result"></div>
        </div>

        <div class="info">
            <p><strong>⏰ Dernière vérification:</strong> <span id="time"></span></p>
    ...
    <script>
        ...
        function testMeta() {
            testEndpoint('/latest.yml', 'Métadonnées');
        }
...
echo "   • Port: $PORT (au lieu de 8080 utilisé par Traefik)"
echo "   • URL: http://$SERVER_IP:$PORT"
echo "   • Protocole: HTTP (pas de SSL)"
echo ""
...
echo "   2. Mettez à jour vos configs avec le port $PORT"
